-- Snapshot product size on sale items for receipt printing.

ALTER TABLE public.sale_items
  ADD COLUMN IF NOT EXISTS size_snapshot text;

COMMENT ON COLUMN public.sale_items.size_snapshot IS 'Product size at time of sale (receipt snapshot).';

UPDATE public.sale_items si
SET size_snapshot = p.size
FROM public.products p
WHERE p.id = si.product_id
  AND si.size_snapshot IS NULL
